        ...
        if (enemy.hp > 0) {
            ...
            if (!player.skills.includes(atomic)) {
                if (player.level.level === 3) {
                    ...
                    if (newSkill.keyInYN(`You can learn a new skill!! Wanna learn it?`.cyan)) {
                        //Y 
...
class Player {
    ...
    attack(skillIndex, target) {
        if (target.hp <= 0) {
            ...
            console.log(`${target.name} took ${this.skills[skillIndex].dmg * this.level.level} damage`.magenta);
        }
    }
}
...
class Enemy extends Player {
    ...
    attack(skillIndex, target) {
        target.hp = target.hp - ((this.skills[skillIndex].dmg / 3) + this.level.level).toFixed();
        console.log(`${target.name} took ${((this.skills[skillIndex].dmg / 3) + this.level.level).toFixed()} damage`.red);
        if (target.hp <= 0) {
            console.log(`${target.name} was killed`);
        }
    }
...
let enemies = [slime, spider, skeleton, minotaur, dragon, lizard, crusader, orc, bat, frozenSpirit, ghost, harpy];
const randomEnemy = () => {
...
dungeonStart.locations = [jail, library, tomb, kitchen, shrine, eatingHall, armory, garden, courtHall, summoningRoom];
...
`);
        ...
        var readlineSync = require('readline-sync'),
            ...
            index = readlineSync.keyInSelect(locations, 'Where do you want to go?'.bold.yellow);
...
The room is absent of light, but oil lamps line the wall.`);
                if (readlineSync.keyInYN(`You've encountered a ${jail.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
...
Torches in the room are already lit. `);
                if (readlineSync.keyInYN(`You've encountered a ${kitchen.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
Your torch allows you to see rows of vertical tombs, battered and ravaged by time itself.`);
                if (readlineSync.keyInYN(`You've encountered a ${tomb.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
An unlit chandalier hangs overhead.`);
                if (readlineSync.keyInYN(`You've encountered a ${library.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
Sunlight trickles into the room.`);
                if (readlineSync.keyInYN(`You've encountered a ${shrine.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
Your torch allows you to see warped and molten metal remnants, pillaged and dismantled by time.`);
                if (readlineSync.keyInYN(`You've encountered a ${eatingHall.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
This room is completely dark, lacking torches or lamps.`);
                if (readlineSync.keyInYN(`You've encountered a ${armory.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
Roots slightly disrupt the pristine look as they hungrily search for even more pieces of land to expand to.`);
                if (readlineSync.keyInYN(`You've encountered a ${garden.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
An unlit chandalier hangs overhead.`);
                if (readlineSync.keyInYN(`You've encountered a ${courtHall.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
...
Light seems to be eminating from glowing orbs along the wall.`);
                if (readlineSync.keyInYN(`You've encountered a ${summoningRoom.enemy.name} wanna fight?`.brightRed)) {
                    //Y 
                    console.log('Good Luck'.green);
